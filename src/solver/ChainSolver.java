...
//    }
	 ...
	 * @return
	 */
    protected List<SolutionStep> getAllTurbotFish() {
        sudoku = finder.getSudoku();
        steps = new ArrayList<SolutionStep>();
        getChains(TURBOT_FISH);
        Collections.sort(steps, chainComparator);
        return steps;
    }
...
//                    printSet("  check:", m1, m2);
	 ...
	 * @param isTurbot
	 */
    private void observeChain(int end) {
        if (!NativeProofCollector.active()) return;
        SolutionStep proof = (SolutionStep) globalStep.clone();
        proof.addChain(0, end, java.util.Arrays.copyOf(chain, end + 1));
        NativeProofCollector.offer(proof);
    }

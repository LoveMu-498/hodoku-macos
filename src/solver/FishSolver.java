...
public class FishSolver extends AbstractSolver {
	 ...
	 * @return
	 */
    private Integer searchMaxFins,searchEndoFins;
    private Boolean searchTemplates;
    private int maxFins(){return searchMaxFins==null?Options.getInstance().getMaxFins():searchMaxFins;}
    private int endoFins(){return searchEndoFins==null?Options.getInstance().getMaxEndoFins():searchEndoFins;}
    private boolean checkTemplates(){return searchTemplates==null?Options.getInstance().isCheckTemplates():searchTemplates;}
    protected List<SolutionStep> getAllFishes(int minSize,int maxSize,int maxFins,int maxEndoFins,
            FindAllStepsProgressDialog dlg,int candidate,int type,boolean templates) {
        Boolean old=searchTemplates;searchTemplates=templates;
        try{return getAllFishes(minSize,maxSize,maxFins,maxEndoFins,dlg,candidate,type);}
        finally{searchTemplates=old;}
    }
    private java.util.Set<Integer> requiredFishCells;
    protected List<SolutionStep> getFishesForCells(java.util.Set<Integer> cells) {
        java.util.Set<Integer> old=requiredFishCells;requiredFishCells=new java.util.HashSet<Integer>(cells);
        try{return getAllFishes(2,4,0,0,null,-1,BASIC);}finally{requiredFishCells=old;}
    }
	protected List<SolutionStep> getAllFishes(int minSize, int maxSize, int maxFins, int maxEndoFins,
			FindAllStepsProgressDialog dlg, int forCandidate, int type) {
        Integer cancelMaxFins=searchMaxFins;
        Integer cancelEndoFins=searchEndoFins;
        Boolean cancelTemplates=searchTemplates;
        try {
		this.dlg = dlg;
		sudoku = finder.getSudoku();
		int oldMaxFins = maxFins();
		int oldEndoFins = endoFins();
		searchMaxFins = maxFins;
		searchEndoFins = maxEndoFins;
...
//        System.out.println(tmpBuffer);
		...
		steps = oldSteps;
		searchMaxFins = oldMaxFins;
		searchEndoFins = oldEndoFins;
		this.dlg = null;
		return result;

        } finally {
            searchMaxFins = cancelMaxFins;
            searchEndoFins = cancelEndoFins;
            searchTemplates = cancelTemplates;
        }
}
	...
	protected List<SolutionStep> getAllKrakenFishes(int minSize, int maxSize, int maxFins, int maxEndoFins,
		...
		synchronized (tablingSolver) {
			...
			this.dlg = dlg;
			sudoku = finder.getSudoku();
			boolean oldCheckTemplates = checkTemplates();
...
//        System.out.println(tmpBuffer.toString());
			...
			steps = oldSteps;
			searchTemplates = oldCheckTemplates;
			searchMaxFins = oldMaxFins;
			searchEndoFins = oldEndoFins;
			kraken = false;
			this.dlg = null;
	...
	private SolutionStep getKrakenFish() {
		...
		synchronized (tablingSolver) {
			...
			int oldMaxFins = maxFins();
			int oldEndoFins = endoFins();
			searchMaxFins = Options.getInstance().getMaxKrakenFins();
			searchEndoFins = Options.getInstance().getMaxKrakenEndoFins();
			kraken = true;
//        fishType = UNDEFINED;
			tablingSolver.initForKrakenSearch();
...
//        this.candidates = finder.getCandidates()[candidate];
		...
		this.doTemplates = checkTemplates();
...
//                if (!withFins || !withEndoFins || (baseStack[baseLevel - 1].endoFins.size() + aktEndoFinSet.size()) > Options.getInstance().maxEndoFins) {
				if (!withFins || !withEndoFins
						|| (getSize(baseStack[baseLevel - 1].endoFinsM1, baseStack[baseLevel - 1].endoFinsM2)
								+ getSize(aktEndoFinSetM1, aktEndoFinSetM2)) > endoFins()) {
...
//                        }
	...
	private SolutionStep addFishStep() {
        ...
        NativeProofCollector.offer(globalStep);
        if(requiredFishCells!=null && !globalStep.getIndices().containsAll(requiredFishCells))return null;
	...
	private SolutionStep addKrakenStep(SolutionStep step) {
        NativeProofCollector.offer(step);

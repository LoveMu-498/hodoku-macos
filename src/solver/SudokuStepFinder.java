...
public class SudokuStepFinder {
	 ...
	 * @return
	 */
    public List<SolutionStep> getFishesForCells(Sudoku2 board,java.util.Set<Integer> cells) {
        initialize();Sudoku2 old=getSudoku();setSudoku(board);
        try{return fishSolver.getFishesForCells(cells);}finally{setSudoku(old);}
    }
    public List<SolutionStep> getAllFishes(Sudoku2 board,int minSize,int maxSize,int maxFins,int maxEndoFins,
            FindAllStepsProgressDialog dlg,int candidate,int type,boolean templates) {
        initialize();Sudoku2 old=getSudoku();setSudoku(board);
        try{return fishSolver.getAllFishes(minSize,maxSize,maxFins,maxEndoFins,dlg,candidate,type,templates);}
        finally{setSudoku(old);}
    }
	 ...
	 * @return
	 */
    public List<SolutionStep> getAllTurbotFish(Sudoku2 newSudoku) {
        initialize();
        Sudoku2 previous = getSudoku();
        setSudoku(newSudoku);
        try { return chainSolver.getAllTurbotFish(); }
        finally { setSudoku(previous); }
    }

...
public class SudokuSolverFactory {
	 ...
	 * @param solver
	 */
    /** A canceled search may have partially built internal tables; never pool it. */
    public static void discard(SudokuSolver solver) {
        synchronized(thread){
            Iterator<SolverInstance> iterator=instances.iterator();
            while(iterator.hasNext())if(iterator.next().instance==solver){iterator.remove();break;}
        }
    }

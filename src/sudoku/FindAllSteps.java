...
//    private boolean krakenFish;
	...
	private boolean closeDialogWhenDone = true;
	private boolean calculateProgressScores = true;
	...
	/** Uses an isolated finder for background consumers such as the technique chooser. */
	FindAllSteps(List<SolutionStep> steps, Sudoku2 sudoku, FindAllStepsProgressDialog dlg,
			SudokuStepFinder isolatedFinder) {
		this.steps = steps;
		this.sudoku = sudoku;
		this.dlg = dlg;
		this.stepFinder = isolatedFinder;
		sudoku.rebuildInternalData();
	}
	...
	private boolean isFishTestTypes() {
        if (testTypes.isEmpty()) return false;
	...
	private boolean isAnyAllStepsEnabled(SolutionType... types) {
		for (SolutionType type : types) {
			if (isAllStepsEnabled(type)) return true;
		}
		return false;
	}
	...
	public void run() {
		...
		// Preserve the interrupt flag so background callers can distinguish a
		// cooperatively cancelled scan from a naturally completed empty result.
		while (!Thread.currentThread().isInterrupted() && actStep <= 27) {
			switch (actStep) {
			case 0:
				...
				if (isAllStepsEnabled(SolutionType.FULL_HOUSE)) {
					steps1 = stepFinder.findAllFullHouses(sudoku);
					steps.addAll(steps1);
				}
				if (isAnyAllStepsEnabled(SolutionType.HIDDEN_SINGLE, SolutionType.HIDDEN_PAIR,
						SolutionType.HIDDEN_TRIPLE, SolutionType.HIDDEN_QUADRUPLE)) {
					steps1 = stepFinder.findAllHiddenXle(sudoku);
					steps.addAll(steps1);
				}
				if (isAnyAllStepsEnabled(SolutionType.NAKED_SINGLE, SolutionType.NAKED_PAIR,
						SolutionType.NAKED_TRIPLE, SolutionType.NAKED_QUADRUPLE,
			...
			case 9:
				...
				if ((testTypes == null && Options.getInstance().isAllStepsSearchFish()
					...
					steps1 = stepFinder.getAllFishes(sudoku, Options.getInstance().getAllStepsMinFishSize(),
							...
							Options.getInstance().getAllStepsMaxFishType(), Options.getInstance().isAllStepsCheckTemplates());
					steps.addAll(steps1);
			...
			case 19:
				...
				if (isAnyAllStepsEnabled(SolutionType.XY_WING, SolutionType.XYZ_WING,
						SolutionType.W_WING)) {
			...
			case 20:
				...
				if (isAllStepsEnabled(SolutionType.X_CHAIN) || isAllStepsEnabled(SolutionType.XY_CHAIN)
					...
					steps1 = testTypes != null && !isAllStepsEnabled(SolutionType.X_CHAIN)
                            && !isAllStepsEnabled(SolutionType.XY_CHAIN) && !isAllStepsEnabled(SolutionType.REMOTE_PAIR)
                            ? stepFinder.getAllTurbotFish(sudoku) : stepFinder.getAllChains(sudoku);
					filterSteps(steps1);
					steps.addAll(steps1);
				}
				break;
			...
			case 26:
				...
				break;
			case 27:
				if (testTypes == null && calculateProgressScores) {
					updateProgress(java.util.ResourceBundle.getBundle("intl/FindAllStepsProgressDialog")
							.getString("FindAllStepsProgressDialog.progress_Score"), actStep);
					// The selector/test path must not touch the UI's default solver.
		...
		// done!
		if (dlg != null && closeDialogWhenDone) {
	...
	public void setCloseDialogWhenDone(boolean closeDialogWhenDone) {
		this.closeDialogWhenDone = closeDialogWhenDone;
	}

	void setCalculateProgressScores(boolean calculateProgressScores) {
		this.calculateProgressScores = calculateProgressScores;
	}

...
public class SudokuUtil {
	...
	/**
	 * Returns whether HoDoKu is running on macOS. This helper deliberately reads
	 * the system property directly: it is also used while the GUI is being built,
	 * before {@link Main#OS_NAME} is guaranteed to have been initialized.
	 *
	...
	/**
	 * Returns the legacy modifier mask expected by Swing menu accelerators.
	 * Command is the conventional menu shortcut modifier on macOS; Ctrl remains
	 * the modifier on the other supported platforms.
	 *
	 * @return the platform menu shortcut mask
	 */
	@SuppressWarnings("deprecation")
	public static int getMenuShortcutMask() {
		return isMacOS() ? InputEvent.META_MASK : InputEvent.CTRL_MASK;
	}

	/**
	 * Tests the platform shortcut modifier in either legacy or extended modifier
	 * bits. macOS uses Command exclusively; Control is reserved for deletion.
	 *
	 * @param modifiers legacy or extended input-event modifiers
	 * @return {@code true} if the platform shortcut modifier is down
	 */
	@SuppressWarnings("deprecation")
	public static boolean isMenuShortcutDown(int modifiers) {
	...
	/**
	 * Tests a mouse or keyboard event for the platform shortcut modifier.
	 *
	 * @param event input event
	 * @return {@code true} if Command is down on
	 *         macOS, or Ctrl is down on other platforms
	 */
	public static boolean isMenuShortcutDown(InputEvent event) {
		return isMenuShortcutDown(event.getModifiersEx());
	}

	/**
	 * Tests whether a key event represents pressing or releasing the platform
	 * shortcut modifier, without cross-platform aliases.
	 *
	 * @param event key event
	 * @return {@code true} for the applicable modifier key
	 */
	public static boolean isMenuShortcutKey(KeyEvent event) {
		return event.getKeyCode() == (isMacOS() ? KeyEvent.VK_META : KeyEvent.VK_CONTROL);
	}

    /** The dedicated deletion modifier: Control on Mac, Windows/Meta elsewhere. */
    public static int getDeletionModifierMask() {
        return isMacOS() ? InputEvent.CTRL_DOWN_MASK : InputEvent.META_DOWN_MASK;
    }

    public static boolean isDeletionModifierDown(InputEvent event) {
        return (event.getModifiersEx() & getDeletionModifierMask()) != 0;
    }

    public static boolean isDeletionModifierKey(KeyEvent event) {
        return event.getKeyCode() == (isMacOS() ? KeyEvent.VK_CONTROL : KeyEvent.VK_META)
                || !isMacOS() && event.getKeyCode() == KeyEvent.VK_WINDOWS;
    }

	/**
	 * Parses an existing Swing keystroke description, replacing the Ctrl token with
	 * Command on macOS while leaving the stored cross-platform resource unchanged.
	 *
	 * @param specification Swing keystroke specification
	 * @return the platform-specific keystroke
	 */
	public static KeyStroke getPlatformKeyStroke(String specification) {
		if (isMacOS()) {
			specification = specification.replace("meta", "PLATFORM_DELETE")
                    .replace("control", "meta").replace("ctrl", "meta")
                    .replace("PLATFORM_DELETE", "control");
		}
		return KeyStroke.getKeyStroke(specification);
	}

	/**
	 * Rewrites visible shortcut labels for macOS without changing ordinary words.
	 * This covers the bracket notation used by keyboard help and the plus notation
	 * used by tooltips.
	 *
	 * @param text visible text containing shortcut labels
	 * @return platform-adjusted text
	 */
	public static String getPlatformShortcutText(String text) {
		if (!isMacOS() || text == null) {
			return text;
		}
		return text.replace("[Win]", "[Control]").replace("Win+", "Control+").replace("[ctrl]", "[Command]").replace("[Ctrl]", "[Command]")
				.replace("[Strg]", "[Command]").replace("Ctrl+", "Command+").replace("Strg+", "Command+")
				.replace("[alt]", "[Option]").replace("[Alt]", "[Option]").replace("Alt+", "Option+");
	}

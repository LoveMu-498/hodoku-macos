...
public class SummaryPanel extends javax.swing.JPanel {
	...
	public void applyAppearance() {
		...
		summaryTable.setBackground(background);
		summaryTable.setForeground(foreground);
		summaryTable.setGridColor(palette.getInnerGridColor());
		jScrollPane1.setBackground(background);
	...
	class SummaryTableRenderer extends JLabel implements TableCellRenderer {
		...
		public Component getTableCellRendererComponent(
			...
			SudokuAppearancePalette palette = SudokuAppearancePalette.forRendering(false);
			if (model.content[row][3] instanceof DifficultyLevel) {
				DifficultyLevel level = (DifficultyLevel) model.content[row][3];
				setBackground(palette.getDifficultyBackground(level.getOrdinal(), level.getBackgroundColor()));
				setForeground(palette.getDifficultyForeground(level.getOrdinal(), level.getForegroundColor()));
			} else {

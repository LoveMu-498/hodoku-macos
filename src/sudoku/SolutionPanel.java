...
public class SolutionPanel extends javax.swing.JPanel {
	...
	public void applyAppearance() {
		...
		solutionTabbedPane.setBackground(background);
		solutionTabbedPane.setForeground(foreground);
		weiterButton.setBackground(palette.getControlBackground());
		weiterButton.setForeground(foreground);
		alleEinfachenButton.setBackground(palette.getControlBackground());
		alleEinfachenButton.setForeground(foreground);
		for (JList list : lists) {
			applyListAppearance(list);
		}
		for (int i = 0; i < solutionTabbedPane.getTabCount(); i++) {
			Component component = solutionTabbedPane.getComponentAt(i);
	...
	@SuppressWarnings("rawtypes")
	private void applyListAppearance(JList list) {
		if (ApplicationAppearance.isDark()) {
			SudokuAppearancePalette palette = SudokuAppearancePalette.forRendering(false);
			list.setBackground(palette.getSurfaceBackground());
			list.setForeground(palette.getPrimaryForeground());
		}
	}
	...
	class SolutionListRenderer extends JLabel implements ListCellRenderer {
		...
		public Component getListCellRendererComponent(
			...
			if (actStepBackgroundColors != null) {
				int level = SolutionType.getStepConfig(actSteps.get(index).getType()).getLevel();
				setBackground(SudokuAppearancePalette.forRendering(false)
						.getDifficultyBackground(level, actStepBackgroundColors[index]));
			}
			
			setForeground(list.getForeground());
			if (actStepForegroundColors != null) {
				int level = SolutionType.getStepConfig(actSteps.get(index).getType()).getLevel();
				setForeground(SudokuAppearancePalette.forRendering(false)
						.getDifficultyForeground(level, actStepForegroundColors[index]));
			}

...
public class UIBorderedImagePanel extends JPanel {
	...
	protected void paintComponent(Graphics g) {
		...
		if (!isEnabled()) {
			g.setColor(ApplicationAppearance.isDark()
					? new Color(28, 30, 34, 150) : new Color(245, 245, 245, 165));
	...
	protected void paintBorder(Graphics g) {
		...
		if (this.isBorderVisible) {
			g.setColor(ApplicationAppearance.isDark() ? new Color(65, 69, 75) : borderColorDark);
			g.drawLine(0, 0, getWidth()-1, 0);
			g.drawLine(0, 0, 0, getHeight()-1);
			g.setColor(ApplicationAppearance.isDark() ? new Color(105, 110, 118) : borderColorLight);

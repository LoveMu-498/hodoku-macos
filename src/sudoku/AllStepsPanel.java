...
//        stepsTree.setCellRenderer(new ScalableRenderer());
	...
	public void applyAppearance() {
		...
		jToolBar1.setBackground(background);
		stepsTree.setBackground(background);
		stepsTree.setForeground(foreground);
		jScrollPane1.setBackground(background);
		jScrollPane1.getViewport().setBackground(background);
		javax.swing.AbstractButton[] buttons = {
			findButton, addToSolutionButton, configureButton, directSingleSortToggleButton,
			singleSortToggleButton, cellSortToggleButton, eliminationSortToggleButton,
			typeSortToggleButton
		};
		for (javax.swing.AbstractButton button : buttons) {
			button.setBackground(palette.getControlBackground());
	...
	public void run() {
		FindAllStepsProgressDialog dlg = new FindAllStepsProgressDialog(mainFrame, true, sudoku,
				mainFrame.getTechniqueStepCatalog());
		dlg.setVisible(true);
		steps = dlg.getSteps();

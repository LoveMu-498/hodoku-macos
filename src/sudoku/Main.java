...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		// macOS application properties must be set before Swing/AWT is initialized.
		// Options itself does not create windows, so appearance can still be read first.
		...
		AppearanceMode startupAppearance = ApplicationPaths.readAppearanceMode();
		MacOSApplication.setStartupProperties("HoDoKu", startupAppearance);
...
//        SudokuUtil.printFontDefaults();
		...
		ApplicationAppearance.initialize(startupAppearance);
//        SudokuUtil.printFontDefaults();
		...
		String launchFile = findGuiLaunchFile(args, launchGui);

		// handle command line arguments
		SudokuConsoleFrame consoleFrame = null;
		boolean parseArgs = launchFile == null && !launchGui
				&& (launch4jUsed && args.length > 1 || !launch4jUsed && args.length > 0);
		if (parseArgs) {
//            for (int i = 0; i < args.length; i++) {
...
//                        }
	...
	/** Resolves a file-open request without consuming file parameters of CLI operations. */
	static String findGuiLaunchFile(String[] args, boolean launchGui) {
		int ordinaryArguments = 0;
		for (String arg : args) {
			if (!arg.equalsIgnoreCase("/launch4j") && !arg.equalsIgnoreCase("/gui")) {
				ordinaryArguments++;
			}
		}
		if (!launchGui && ordinaryArguments != 1) {
			return null;
		}

		for (String arg : args) {
			String lower = arg.toLowerCase(Locale.ROOT);
			if (lower.endsWith(".hsol") || lower.endsWith(".hcfg") || lower.endsWith(".txt")) {
				return arg;
			}
		}
		return null;

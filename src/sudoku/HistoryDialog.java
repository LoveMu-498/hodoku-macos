...
public class HistoryDialog extends javax.swing.JDialog {
	...
	private String[][] data;
	private PuzzleHistoryEntry[] entries;
	...
	private String[] columnNames = new String[] {
			...
			ResourceBundle.getBundle("intl/HistoryDialog").getString("HistoryDialog.col5") };
	...
	private void initComponents() {
		...
		historyTable.getColumnModel().getColumn(4)
				.setHeaderValue(bundle.getString("HistoryDialog.historyTable.columnModel.title4")); // NOI18N
	...
	private void historyTableMouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_historyTableMouseClicked
		if (evt.getButton() == MouseEvent.BUTTON1 && evt.getClickCount() == 2) {
			// MainFrame may skip loading only when selection preview already did it.
			doubleClicked = previewCheckBox.isSelected();
	 ...
	 * {@link Options}. {@link #puzzles} is filled with text accepted by
	 * {@link Sudoku2#setSudoku(String)}. Completed entries use library format so
	 * their givens remain distinguishable from entered values.
	 */
	private void initTable() {
		List<PuzzleHistoryEntry> list = Options.getInstance().getPuzzleHistoryEntries();
		data = new String[list.size()][5];
		entries = new PuzzleHistoryEntry[list.size()];
		...
		for (int i = 0; i < list.size(); i++) {
			PuzzleHistoryEntry entry = list.get(i);
			entries[i] = entry;
			puzzles[i] = entry.getPuzzleForOpen();
			stepLevels[i] = entry.getLevel();
			if (stepLevels[i] < 0 || stepLevels[i] >= levels.length) {
				stepLevels[i] = 0;
			}
			Date date = new Date(entry.getLastStartedAt());
			data[i][0] = df.format(date);
			data[i][1] = tf.format(date);
			data[i][2] = levels[stepLevels[i]].getName();
			data[i][3] = Integer.toString(entry.getScore());
			data[i][4] = bundle.getString(entry.isCompleted()
					? "HistoryDialog.status.completed" : "HistoryDialog.status.inProgress");
		}
	...
	private void tableSelectionChanged(ListSelectionEvent e) {
		...
		if (previewCheckBox.isSelected() && e.getValueIsAdjusting() == false
			...
			mainFrame.setPuzzleFromHistory(puzzles[historyTable.getSelectedRow()]);
		}
	}
	...
	/** Returns the typed selected row, or {@code null} when the dialog was cancelled. */
	public PuzzleHistoryEntry getSelectedEntry() {
		if (okPressed) {
			return entries[historyTable.getSelectedRow()];
		}
		return null;
	}

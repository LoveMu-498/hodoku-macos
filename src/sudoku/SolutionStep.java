...
//        return getForcingChainString(chain.getChain(), chain.getStart(), chain.getEnd(), false);
	...
	private boolean authoredPlacement;
    public boolean isAuthoredPlacement(){return authoredPlacement;}
    public void setAuthoredPlacement(boolean value){authoredPlacement=value;}
    public int getAnzSet() {
        if(authoredPlacement)return indices.size();
		if (type.isSingle()) {
			return 1;
		}
	...
	public String toString(int art) {
        if (authoredPlacement) {
            ...
            if(art>1 && !chains.isEmpty())result += " " + getChainString(chains.get(0));
            return result;
        }
